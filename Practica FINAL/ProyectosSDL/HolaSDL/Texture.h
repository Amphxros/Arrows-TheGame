...
public:
	...
	Texture::Texture(SDL_Renderer* r) : renderer(r) {};
	Texture::Texture(SDL_Renderer* r, string filename, uint numRows = 1, uint numCols = 1) : renderer(r) { load(filename, numRows, numCols); };
	~Texture() { Clear(); };
	void Clear();
